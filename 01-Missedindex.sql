SELECT
    ...
    'CREATE INDEX index_' + PARSENAME(REPLACE(REPLACE(mid.statement, '[', ''), ']', ''), 1) +
    ' ON ' + mid.statement + ' (' +
...
OUTER APPLY (
    SELECT TOP (1) us.user_updates
    FROM sys.dm_db_index_usage_stats us
    WHERE us.database_id = mid.database_id
      AND us.object_id = mid.object_id
      AND us.index_id IN (0, 1)   -- heap / clustered
    ORDER BY CASE WHEN us.index_id = 1 THEN 0 ELSE 1 END
) ius
