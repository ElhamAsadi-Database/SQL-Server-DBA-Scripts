...
CREATE OR ALTER PROCEDURE [dbo].[View_dm_os_buffer_descriptors] as
SELECT 
...
CREATE OR ALTER PROCEDURE [dbo].[IOPS_Detailes] AS 
-- create table 
...
SELECT  
   ...
   (readsBytes2 - readsBytes1)/1024 AS readsKBytesDiff, 
   readsIoStall2 - readsIOStall1 AS readsIOStallDiff, 
   writesNum2 - writesNum1 AS writesNumDiff, 
   (writesBytes2 - writesBytes1)/1024 AS writesKBytesDiff, 
   writesIoStall2 - writesIOStall1 AS writesIOStallDiff,    
   ioStall2 - ioStall1 AS ioStallDiff ,
   ((readsBytes2 - readsBytes1)/1024)+((writesBytes2 - writesBytes1)/1024 )  as throughput,
   (readsNum2 - readsNum1)+(writesNum2 - writesNum1) as iops
...
CREATE OR ALTER PROCEDURE [dbo].[MonitorCurrentQuery]
AS
    BEGIN
        SELECT  DISTINCT requests.session_id ,
                ...
                CAST(requests.total_elapsed_time AS MONEY) / 60000 AS minDuration ,
                sessions.program_name
        FROM    sys.dm_exec_requests requests
                INNER JOIN sys.dm_exec_sessions sessions ON requests.session_id = sessions.session_id
                LEFT JOIN sys.dm_exec_connections connections ON requests.session_id = connections.session_id
                OUTER APPLY sys.dm_exec_sql_text(requests.sql_handle) sql_text
        WHERE   sql_text.text IS NOT NULL
                AND sql_text.text NOT LIKE N'%requests.session_id ,        sessions.login_name ,        connections.client_net_address ,        sessions.host_name ,%'
...
USE [master]
GO
CREATE OR ALTER PROCEDURE [dbo].[sp_help_revlogin] @login_name sysname = NULL AS
